...
export interface HorizontalRefs {
  wrap: HTMLElement
  track: HTMLElement
}

/**
 * PHASE 2 (scrollProgress 1 -> 2): the same continuous system, extended. A
 * pinned, scrubbed ScrollTrigger maps vertical scroll to horizontal translateX
 * of the panel track; per-panel parallax adds depth via containerAnimation.
 */
export function buildHorizontalTimeline(
  refs: HorizontalRefs,
  report?: ProgressReporter,
) {
  const { wrap, track } = refs
  ...
  const tween = gsap.to(track, {
    ...
    scrollTrigger: {
      ...
      anticipatePin: 1,
      invalidateOnRefresh: true,
      onUpdate: (self) => report?.(1 + self.progress),
    },
  })
